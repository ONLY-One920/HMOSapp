...
def update_cart_quantity(current_user):
    ...
    if not cart_item:
        current_app.logger.error(f'购物车项未找到: item_id={item_id} user_id={current_user.id}')
...
def get_ai_messages(current_user):
    messages = AIMessage.query.filter_by(user_id=current_user.id).order_by(AIMessage.timestamp).all()
    return jsonify([{
        'id': msg.id,
        'role': msg.role,
        'content': msg.content,
